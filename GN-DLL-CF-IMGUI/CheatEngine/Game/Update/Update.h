...
#define Mtrix 0x140558560										//矩阵						特征码：48 8B 0D ?? ?? ?? ?? 48 8D ?? ?? 41 B8 ?? ?? ?? ?? 48 8B 01 FF 90 ?? ?? ?? ??    cshell_x64.dll模块上：mov rcx,[cshell_x64.dll+0xXXXXXXX] '矩阵地址 = (cshell_x64.dll+0xXXXXXXX)的值 + 0x2D00 - 0x10
...
#define PlayerArrayBaseAddress 0x2F07EA0						//玩家数组
#define ModelOffset 0x2F093FC									//判断模式
#define ModelOffset2 0x2F07DDF									//模式基址
...
#define Hitchaddress 0x1403DE6E4								//绘制挂接地址				特征码：45 33 C9 45 33 C0 33 D2 E9 ?? ?? ?? ?? FF 15 ?? ?? ?? ?? 8B D8 
#define DrawIndexedPrimitiveaddress 0x140254CD7					//渲染挂接地址				特征码：44 89 64 24 20 FF 90 ?? ?? 00 00 83 3D ?? ?? ?? ?? 02 7C ?? 8B 84 24 ?? ?? ?? ?? 按顺序数第十个地址
#define SilentTrackAddress 0x14024FB70 + 0x10					//静默追踪构造				特征码：48 83 EC 28 0F 28 C2 4C 8B CA F3 0F 10 54 24 ??
#define RedNameTrackAddress 0x14024C508							//红名追踪地址				特征码：49 8B C0 4C 8B CA 4C 8D 41 ?? 48 8B D0 49 8B C9 E9 ?? ?? ?? ?? CC CC	jmp 跳转的函数既红名追踪地址
#define BarrierOffset 0x140050860								//障碍判断基址				特征码：4C 8B C9 4C 8B C2 48 8B ?? ?? ?? ?? ?? 49 8B D1 48 8B 01 48 FF ?? 18
#define SpaceContinuousJumpOffset 0x317BAB8						//空格连跳					特征码：44 0F B6 3D ?? ?? ?? ?? 8B CB FF 90 ?? ?? ?? ?? 84 C0   movzx r15d,byte ptr [连跳地址]
#define KnifeDistanceBaseAddress 0x30B1E60						//刀距基址 | 武器基址
...
#define SelfAddressOffset 0x28A									//本人地址偏移 本人ID
#define CharacterDataOffset 0x290								//OBJECT | 人物数据 | 玩家数据
...
///////////////////////Hook功能///////////////////////
#define BulletWithoutBackSeatHookOffset 0xAADE4D										//静态无后Hook地址			特征码：F3 44 0F 10 05 ?? ?? ?? ?? 0F 28 F7 F3 0F 59 BC B7 ?? ?? ?? ?? F3 0F 59 B4 B7 ?? ?? ?? ?? F3 41 0F 59 F8 F3 41 0F 59 F0
...
#define BiochemicalCampOffset 0x1145F							//生化阵营偏移 | 玩家阵营
#define BoneOffset 0x28F8										//骨骼偏移
