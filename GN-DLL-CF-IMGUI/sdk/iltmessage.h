...
public:
	...
	virtual void Reset() = 0;//30

	// Return an ILTMessage_Read initialized with this message's data
	// Note : This will reset the message
	virtual ILTMessage_Read *Read() = 0;//38

	// Number of bits which have been written to this message
	virtual uint32 Size() const = 0;//40

	// Basic data writing functions
	// Note : nSize is in bits
	virtual void WriteBits(uint32 nValue, uint32 nSize) = 0;//48
	virtual void WriteBits64(uint64 nValue, uint32 nSize) = 0;//50
	virtual void WriteData(const void *pData, uint32 nSize) = 0;//58

	// Complex data type writing functions
	virtual void WriteMessage(const ILTMessage_Read *pMsg) = 0;//60
	// Exactly like WriteMessage, only without the size indicator.  (So it can't be read with ReadMessage.)
	virtual void WriteMessageRaw(const ILTMessage_Read *pMsg) = 0;//68
	virtual void WriteString(const char *pString) = 0;//70
	virtual void WriteWString(const wchar_t *pwString) = 0;//78
	virtual void WriteCompLTVector(const LTVector &vVec) = 0;//80
	//virtual void WriteBits33(uint64 nValue, uint32 nSize) = 0;//80
	virtual void WriteCompPos(const LTVector &vPos) = 0;//88
	virtual void WriteCompLTRotation(const LTRotation &cRotation) = 0;//90
	virtual void WriteObject(HOBJECT hObj) = 0;//98
	virtual void WriteYRotation(const LTRotation &cRotation) = 0;//A0
	virtual void WriteDatabaseRecord( IDatabaseMgr *pDatabase, HRECORD hRecord ) = 0;//A8
	virtual void WriteTimer(HENGINETIMER hTimer) = 0;//B0
	virtual void WriteBits22(uint64 nValue, uint32 nSize) = 0;//B8
	...
	int AddCout(__int64 address)
	{
		UCHAR edx = *(UCHAR*)address;
		int ebx = edx << 4;							//mov ebx, edx  |  shl rbx, 04
		UCHAR eax = edx + 1;						//lea eax, [rdx + 1]  °´Î»+1
		*(UCHAR*)address = eax;						//mov [rcx], al
		eax = edx >> 4;								//mov eax, edx  |  shr rax, 04
		ebx = ebx | eax;							//or rbx, rax
		return ebx;
	}
};
